...
url = "http://qa-assignment.oblakogroup.ru/board/anastasiya_surina_" + str(new_url_ending(6))


@given(u'Открыть страницу')
...
@given(u'Ввести название задачи "{text}"')
...
@given(u'Очистить поле название задачи')
def step_impl(context):
    task = context.driver.find_element_by_id('project_text')
    task.clear()
...
@given(u'Дать новое название категории - "{text}"')
def step_impl(context, text):
...
@given(u'Очистить поле новое название категории')
def step_impl(context):
    new_category = context.driver.find_element_by_id('project_title')
    new_category.clear()


@given(u'Нажать на кнопку OK')
def step_impl(context):
...
@given(u'Нажать на кнопку Отмена')
def step_impl(context):
    hide_button = context.driver.find_element_by_xpath('//*[@id="hide_new_todo"]/div')
    context.driver.execute_script("arguments[0].click();", hide_button)
